        ...
        <div class="modal-content">
            <form role="form" id="form-filter" class="form-horizontal">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-md-9">
                            <div class="input-group">
                                <input type="text" class="form-control date-picker" name="date_from" id="date_from"
                                    placeholder="From" data-date-format="dd-mm-yyyy" autocomplete="off">
                                <span class="input-group-addon"><b>s/d</b></span>
                                <input type="text" class="form-control date-picker" name="date_to" id="date_to"
                                    placeholder="To" data-date-format="dd-mm-yyyy" autocomplete="off">
                            </div>
                        </div>
                    </div>
                    <div class="form-group">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-warning" id="btn-filter"><i class="fa fa-search"></i>
                        Filter</button>
                    <button type="button" class="btn btn-default" id="btn-reset"><i class="fa fa-refresh"></i>
                        Reset</button>
                </div>
            </form>
        </div>
                    ...
                    <div class="portlet-title">
                        ...
                        <div class="actions">
                            <a data-toggle="modal" data-target="#filterData">
                                <button type="button" class="btn btn-warning btn-xs"><i class="fa fa-search"></i> Filter
                                    Data</button>
                            </a>
                        </div>
                    </div>
...
$(document).ready(function() {
    table = $('#tableData').DataTable({
        ...
        "order": [],
        "ajax": {
            "url": "<?=site_url('kasir/order/data_list')?>",
            "type": "POST",
            "data": function(data) {
...
function konfirmData(order_id) {
    ...
    swal({
        ...
        if (!isConfirm) return;
        $.ajax({
            url: "<?=site_url('kasir/order/confirmdata')?>/" + id,
            type: "POST",
...
function hapusData(order_id) {
    ...
    swal({
        ...
        if (!isConfirm) return;
        $.ajax({
            url: "<?=site_url('kasir/order/deletedata')?>/" + id,
            type: "POST",
            success: function(data) {
                swal({
                    title: "Sukses",
