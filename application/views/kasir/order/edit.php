        ...
        <div class="page-bar">
            <ul class="page-breadcrumb">
                <li>
                    ...
                    <a href="<?= site_url('kasir/home'); ?>">Dashboard</a>
                ...
                <li>
                    <a href="<?= site_url('kaasir/order'); ?>">Order</a>
                    <i class="fa fa-angle-right"></i>
                </li>
                <li>
                    ...
                    <div class="portlet-body form">
                        <form id="formInput" name="formInput" method="post" class="form-horizontal form">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-md-5">
                                                <div class="input-icon right">
                                                    <i class="fa"></i>
                                                    <input type="text" class="form-control" name="menu_kode"
                                                        id="menu_kode" placeholder="Cari Menu" autocomplete="off"
                                                        autofocus />
                                                </div>
                                            </div>
                                            <div class="col-md-2">
                                                <a data-toggle="modal" data-target="#tableDataMenu"
                                                    class="btn btn-danger" id="btn-search">
                                        ...
                                        <div class="form-group form-md-line-input">
                                            ...
                                            <div class="col-md-9">
                                                <input type="text" class="form-control" name="menu_nama" id="menu_nama"
                                                    placeholder="Nama Menu" autocomplete="off" readonly />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <div class="col-md-4">
                                                <div class="input-icon right">
                                                    <i class="fa"></i>
                                                    <input type="text" class="form-control number" name="qty" id="qty"
                                                        placeholder="0" autocomplete="off" onkeydown="hitungTotal()" />
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <div class="col-md-4">
                                                <div class="input-icon right">
                                                    <i class="fa"></i>
                                                    <input type="text" class="form-control number" name="harga"
                                                        id="harga" placeholder="0" autocomplete="off"
                                                        onkeydown="hitungTotal()" />
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div class="form-actions" align="center">
                                ...
                                <button type="submit" class="btn btn-success"><i class="fa fa-floppy-o"></i> Simpan
                                    Item</button>
                                <a onclick="bayarOrder(<?= $detail->order_id; ?>)" type="button"
                                    class="btn btn-primary"><i class="fa fa-money"></i> Bayar</a>
                                <?php } else { ?>
                                <a onclick="printNota(<?= $detail->order_id; ?>)" type="button"
                                    class="btn btn-primary"><i class="fa fa-print"></i> Print Nota</a>
                                <?php } ?>
                                <a href="<?= site_url('kasir/order'); ?>" type="button" class="btn btn-warning"><i
...
<script type="text/javascript"
    src="<?= base_url('backend/assets/global/plugins/datatables/plugins/bootstrap/dataTables.bootstrap.js'); ?>">
</script>
<script type="text/javascript"
...
$(document).ready(function() {
    tableMenu = $('#tableMenu').DataTable({
        ...
        "order": [],
        "ajax": {
            "url": "<?= site_url('kasir/order/data_menu_list') ?>",
            "type": "POST"
        },
        "columnDefs": [{
            "targets": [0, 1, 4, 5, 6],
...
$(document).ready(function() {
    ...
    tableOrder = $('#tableOrder').DataTable({
        ...
        "order": [],
        "ajax": {
            "url": "<?= site_url('kasir/order/data_order_list/') ?>" + order_id,
            "type": "POST"
        },
        "columnDefs": [{
    ...
    $("#formInput").validate({
        ...
        submitHandler: function(form) {
            ...
            if (statusinput == 'Tambah') {
                $.ajax({
                    url: "<?= site_url('kasir/order/saveitem'); ?>",
                    type: "POST",
                    data: dataString,
                    dataType: 'JSON',
                ...
                $.ajax({
                    url: "<?= site_url('kasir/order/updateitem'); ?>",
                    type: "POST",
                    data: dataString,
                    dataType: 'JSON',
                    success: function(data) {
...
function getTotal() {
    var order_id = '<?= $detail->order_id; ?>';
    $.ajax({
        url: "<?= site_url('kasir/order/get_data_total/'); ?>" + order_id,
        type: "GET",
        dataType: "JSON",
        success: function(data) {
            var Waktu;
...
function editData(id) {
    statusinput = 'Edit';
    $.ajax({
        url: "<?= site_url('kasir/order/get_data_detail/'); ?>" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data) {
            var locale = 'en';
...
function hapusData(order_detail_id) {
    ...
    swal({
        ...
        if (!isConfirm) return;
        $.ajax({
            url: "<?= site_url('kasir/order/deletedataitem') ?>/" + id,
            type: "POST",
            success: function(data) {
                swal({
                    title: "Sukses",
...
function bayarOrder(id) {
    $('#formBayar')[0].reset();
    $.ajax({
        url: "<?= site_url('kasir/order/get_data/'); ?>" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data) {
...
$(document).ready(function() {
    ...
    $("#formBayar").validate({
        ...
        submitHandler: function(form) {
            ...
            $.ajax({
                url: '<?= site_url('kasir/order/bayar'); ?>',
                type: "POST",
                data: dataString,
                success: function(data) {
                    swal({
                    ...
                    window.location = "<?= site_url('kasir/order'); ?>";
                },
                error: function() {
                    swal({
                        title: "Error",
...
function printNota(order_id) {
    var url = "<?= site_url('kasir/order/cetaknotabayar/'); ?>" + order_id;
