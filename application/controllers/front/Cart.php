...
class Cart extends MY_Controller
{
    ...
    public function additem()
    {
        if ($this->input->post()) {
            ...
            foreach ($data as $d_key => $d_value) {
                ...
                // 🔥 Tambahkan ini
                $data[$d_key]['stok']    = $d_value['stok_menu'];
            }
    ...
    public function update_item()
    {
        ...
        if ($this->input->post('rowid') && $this->input->post('qty')) {
            ...
            // cari item cart berdasarkan rowid
            $item = $this->cart->get_item($rowid);

            if ($item) {

                // ambil stok menu dari database
                ...
                if ($menu) {
                    ...
                    if ($qty > $menu->stok_menu) {

                        // STOK TIDAK CUKUP
                        $res['status']  = false;
                        $res['message'] = 'Stok menu tidak cukup. Stok tersedia: ' . $menu->stok_menu;
                    } else {
                        ...
                        $cart_content = $this->cart->contents();
                        foreach ($cart_content as $key => $value) {
                            $cart_content[$key]['price_sub_format'] =
                                number_format($cart_content[$key]['subtotal'], 0, '', ',');
                        }

                        $res['cart_content']      = $cart_content[$rowid];
    ...
    public function clear($meja_id)
    {
        $this->db->where('meja_id', $meja_id);
        $this->db->delete('cart_meja');
    }
}
