...
class Order extends MY_Controller
{
    ...
    public function data_list()
    {
        ...
        foreach ($List as $r) {
            ...
            $row[] = $r->order_nama;
            $row[] = $r->meja_nama;
            $row[] = date('d-m-Y H:i', strtotime($r->order_waktu));
    ...
    public function bayar()
    {
        ...
        // ================= VALIDASI AWAL =================
        if (!$order_id) {
            show_error('Order ID tidak valid');
        }

        // ================= AMBIL DATA ORDER (UNTUK MEJA) =================
        $order = $this->db
            ->select('order_id, meja_id')
            ->get_where('resto_order', ['order_id' => $order_id])
            ->row();

        if (!$order) {
            show_error('Data order tidak ditemukan');
        }
        ...
        // ================= UPDATE DETAIL ORDER =================
        $this->db->where('order_id', $order_id);
        $this->db->update('resto_order_detail', [
        ...
        foreach ($listItem as $i) {
            $menu = $this->db
                ->get_where('resto_menu', ['menu_id' => $i->menu_id])
                ->row();

            if ($menu) {
                $this->db->where('menu_id', $i->menu_id);
                $this->db->update('resto_menu', [
                    'menu_jual' => $menu->menu_jual + $i->order_detail_qty
                ]);
            }
        }

        // ================= KUNCI UTAMA: BUKA KEMBALI MEJA =================
        if (!empty($order->meja_id)) {
            $this->db->where('meja_id', $order->meja_id);
            $this->db->update('resto_meja', [
                'meja_status' => 0 // 0 = KOSONG
            ]);
        }
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            show_error('Pembayaran gagal, data dibatalkan');
        } else {
